"""This file is just to make last-minute circadian plots (it's a mess)."""
...
from cycle_extraction.cycle_extraction_plots import make_filtered_feature_plots
from cycle_extraction.cycle_functions import nc_filter_circadian
...
def _compute_plv_metrics_for_ptnt(
    ...
    for chunk in chunked_segs:
        ...
        for feat in feature_names:
            # Apply the filter
            x = chunk[feat].values
            filt_chunk[feat] = nc_filter_circadian(x, fs=segs_per_hour, order=10)
...
def cycle_extraction_for_pdir(
    ...
    logging.info(f'[{pdir.name}] 🚀 Starting Circadian Cycle Extraction...')
    ...
    make_filtered_feature_plots(seg_feats, seg_feats_filt, event_timestamps, feature_names,
                                pdir.cycle_extraction_dir / 'filtered_feature_plots_circadian',
                                test_start_mtz)

    logging.info(f'[{pdir.name}] ✅ Completed Circadian Cycle Extraction Plots.')
