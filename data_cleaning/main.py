...
from config import PROBLEMATIC_EDFS_FOLDER, PROBLEMATIC_EDFS_FILE, REMAINING_DUPLICATES_FILE, \
    BASE_PATH, DATA_CLEANING_LOG_FOLDER, UNEEG_EXTENDED_DIR, FOR_MAYO_DIR, COMPETITION_DIR
...
def data_cleaning():
    ...
    input(f"Cleaning for {BASE_PATH}. Press enter to continue.")
    DATA_CLEANING_LOG_FOLDER.mkdir(exist_ok=True)

    logging.basicConfig(level=logging.INFO)

    logging.info('========== file_correction ==========')
    removed_duplicates = file_correction(BASE_PATH)
    ...
    problematic_edfs = rename_and_move_edf_data([FOR_MAYO_DIR, UNEEG_EXTENDED_DIR], PROBLEMATIC_EDFS_FOLDER)
